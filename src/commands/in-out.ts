    ...
    async execute(client: client, interaction: CommandInteraction) {
        ...
        if (_setup != "yes") {
            await interaction.reply({
                embeds: [
                    {
                        color: 0xE51F33,
                        description: `🟥 **Cancle in-out report** ที่ห้องไอดี: **${interaction.channelId}** เรียบร้อยแล้วเจ้าค่ะ`
                    }
                ]
            })
        ...
        await interaction.reply({
            embeds: [
                {
                    color: 0x0099ff,
                    description: `🟦 **Setup in-out report** ที่ห้องไอดี: **${interaction.channelId}** เรียบร้อยแล้วเจ้าค่ะ
